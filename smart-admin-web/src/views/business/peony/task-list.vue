    ...
    <div>
        <Card class="smart-query-card">
            ...
            <Row class="smart-query-form-row">
                <span>
                    ID :
                    <Input placeholder="请输入ID" style="width: 180px" v-model="queryForm.projectId" />
                </span>
                <span>
                    名字 :
                    <Input placeholder="请输入名字" style="width: 180px" v-model="queryForm.projectName" />
                </span>
                <ButtonGroup>
                    <Button
                            @click="queryList"
        ...
        <Card class="warp-card">
            ...
            >
                 <template slot-scope="{ row, index }" slot="imageUrl">
                     <img width="40" height="40" v-if="index % 2 === 0" src="https://img0.baidu.com/it/u=3824524318,1251548172&fm=26&fmt=auto&gp=0.jpg"/>
...
import { labelTaskApi } from '@/api/label-task';
import via from './components/peony-list-form';
      ...
      mainTable: {
        ...
        columnArray: [
          ...
          },
          {
            title: '创建时间',
            key: 'createTime',
            tableColumn: 'project.create_time',
            sortable: 'custom'
          },
          {
            title: '任务图片数量',
            key: 'imageSize',
            tableColumn: 'image_size',
            sortable: 'custom'
          },
          {
            title: '标注人',
  ...
  methods: {
    ...
    async queryList( ) {
      ...
      try {
        ...
        let result = await labelTaskApi.queryLabelTask(params);
    ...
    showEditPeonyForm(updateObject){
      //t =this.saveModal;
      //alert(updateObject.taskId);
      window.open('http://10.66.33.113:8082/' + '?taskId=' + updateObject.taskId.toString());
      //window.open('/home/zhoup/IdeaProjects/via/via-src-2.0.11/src/index.html' + '?taskId=' + updateObject.taskId.toString());
